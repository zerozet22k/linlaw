...
type Props = {
  ...
  relatedBusinesses: RelatedBusinessAPI[];
  newsletters: INewsletterAPI[];
};

const HomePageContent: React.FC<Props> = ({ data, relatedBusinesses, newsletters }) => {
  const { language } = useLanguage();

  const heroSlides = data[K.HERO_BANNER] ?? [];
  const promoShowcase = data[K.PROMO_SHOWCASE] ?? {};
  const relatedBusinessesCfg = data[K.RELATED_BUSINESS] ?? {};
...

    {
      ...
      node: (
        <RelatedBusinessesSection
          section={relatedBusinessesCfg.section}
          limit={6}
